document.addEventListener('DOMContentLoaded', () => {
    ...
    function startCountdown(prayerData) {
        ...
        countdownInterval = setInterval(() => {
            ...
            if (!nextPrayer) {
                ...
                const nextDayPrayerTime = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, hour, minute);
    ...
    async function searchCity() {
        ...
        try {
            const response = await fetch(`${KEMENAG_API_BASE_URL}/sholat/kota/cari/${query}`);
        ...
        if ("geolocation" in navigator) {
            ...
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                try {
                    ...
                    const city = data.address.city || data.address.state || 'Lokasi Tidak Dikenal';
                    
                    
                    const searchResponse = await fetch(`${KEMENAG_API_BASE_URL}/sholat/kota/cari/${city.split(' ')[0]}`);
                    ...
                    console.error("Error with reverse geocoding or city search:", error);
                    getPrayerTimes('1301', 'Kota Jakarta'); 
                    alert('Gagal mendapatkan nama lokasi, menampilkan jadwal untuk Jakarta.');
                }
            }, (error) => {
                console.error("Geolocation error:", error);
                alert('Gagal mengakses lokasi. Pastikan Anda mengizinkan akses lokasi. Menampilkan jadwal untuk Jakarta.');
                getPrayerTimes('1301', 'Kota Jakarta'); 
            });
        } else {
            alert("Browser Anda tidak mendukung Geolocation. Menampilkan jadwal untuk Jakarta.");
            getPrayerTimes('1301', 'Kota Jakarta'); 
        }
    }
